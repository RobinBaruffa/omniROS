...
//ros::Publisher chatterClock("/clock", &clock_msg);
...
void setup()
{
  Serial.begin(9600);
  setupWiFi();
  //delay(500); //default 2000
  nh.getHardware()->setConnection(server, serverPort);
  nh.initNode();
  nh.advertise(chatter);
  //nh.advertise(chatterClock);
  nh.subscribe(sub);
  nh.subscribe(subPID);
...
void loop() {
  ...
  */
  //clock_msg.clock = newTime;
  /*
    if(millis()-timer > (float)1000/ODOMETRY_PUBLISHER_FREQUENCY){
    timer = millis() + (float)ODOMETRY_PUBLISHER_FREQUENCY/1000 ;
    chatter.publish( &odometry_msg );
    chatterClock.publish(&clock_msg);
